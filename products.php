...
if (mysqli_num_rows($exec) > 0) {
    ...
    while ($category = mysqli_fetch_assoc($exec)) {
        ...
        /*$run_query = "SELECT COUNT(*) AS count_items FROM products WHERE cat_id=$i";*/
        $run_query = "SELECT COUNT(*) AS count_items FROM products WHERE product_category='$cat_name'";
...
if (mysqli_num_rows($exec) > 0) {
    ...
    while ($country = mysqli_fetch_assoc($exec)) {
        $cont_id = $country["cont_id"];
        $cont_name = $country["cont_title"];

        /*$run_query = "SELECT COUNT(*) AS count_items FROM products WHERE cont_id=$i";*/
        $run_query = "SELECT COUNT(*) AS count_items FROM products WHERE product_country='$cont_name'";
        $query = Query($run_query);
        $row = mysqli_fetch_array($query);
        $count = $row["count_items"];
        $i++;
...
if ($page >= 1) {
    $showPost = ($page * 9) - 9;
    ?>
								<div class="pad"><span class="product-qty">Showing <?php echo $showPost + 1; ?>-<?php echo $results; ?> of all products</span></div>
<?php
}
?>
